...
const Footer = () => {
    ...
    return (
        <footer className="mx-auto w-full flex items-center gap-6 border-t py-6 pb-28 sm:pb-6 border-foreground/25 bg-background/95 px-3 shadow-md supports-[backdrop-filter]:bg-background/60 shadow-md supports-[backdrop-filter]:bg-background/60 supports-[backdrop-filter]:bg-clip-padding supports-[backdrop-filter]:backdrop-blur sm:justify-between sm:rounded-none sm:px-3">
            ...
            <div className="grid grid-cols-5 gap-x-6 mx-auto max-w-6xl">
                ...
                <div className="space-y-4 sm:col-span-1">
                    ...
                    <ul className="space-y-4">
                        {
                            flattenedLinks.map(link => <li key={link.href}><Link href={link.href}>{link.title}</Link></li>)
                        }
                    </ul>
                </div>
                <div className="space-y-4 sm:col-span-2">
                    ...
                    <ul className="space-y-4">
                        <li className="flex flex-wrap gap-x-2">
                            <MapPin className="w-5 h-5 text-muted-foreground pt-2 flex-shrink-0" />
                            {defaultUser?.address}
                        </li>
                        <li className="flex flex-wrap gap-x-2">
                            <Phone className="w-5 h-5 text-muted-foreground pt-2 flex-shrink-0" />
                            {defaultUser?.phone}
                        </li>
                        <li className="flex flex-wrap gap-x-2">
                            <Mail className="w-5 h-5 text-muted-foreground pt-2 flex-shrink-0" />
                            {defaultUser?.email}
                        </li>
                    </ul>
