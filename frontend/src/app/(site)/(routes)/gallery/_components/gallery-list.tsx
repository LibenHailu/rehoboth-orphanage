...
export function GalleryList({ data }: GalleryListProps) {
    ...
    function handleOnKeyDown(event: KeyboardEvent) {
        ...
        const currentIndex = data?.gallaryItem.findIndex(
            (gallery) => gallery?.id === activeImage?.id
        );
    ...
    return (
        <>
            ...
            <ul className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {data.gallaryItem?.map((gallery: GalleryItemType) => {
                    return (
                        <li
                            key={gallery?.id}
                            className="group relative overflow-hidden rounded-lg inline"
                        >
            ...
            <dialog
                ref={dialogRef}
                className="relative max-w-[90vw] flex flex-col items-center overflow-visible backdrop:bg-black/85 bg-transparent"
            >
                <div
                ...
                {activeImage && (
                    <div className="absolute w-full bottom-0 py-4 bg-gray-900/70 flex items-center justify-center">
                        <div className="text-white text-center space-y-2">
                            <h3 className="text-lg font-semibold overflow-clip">
                                {activeImage?.title}
                            </h3>
